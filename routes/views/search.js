...
exports = module.exports = function(req, res) {
	...
	function generate (i) {
		...
		var blank_template_path = base_dir + '/blank_template' + i + '.html';
		swig.renderFile(blank_template_path, house_properties_combined, function(err, output) {
			if (err) throw err;
			fs.readFile(base_dir + '/website' + i + '.zip', function(err, data) {
				  if (err) throw err;
				  var zip = new JSZip(data);
				  zip.file('/website'+i+'/web.html',output);
				  var generatedFile = zip.generate({type:"nodebuffer"});
				  fs.writeFile('public/generated/'+req.body.MLS+'/website'+i+'.zip', generatedFile, function(err) {
					if (err) throw err;
				  });
			});
		});
